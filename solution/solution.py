...
def call_openai(
    ...
    return str(response_text), float(latency)
...
def compare_models(prompt: str) -> dict:
    ...
    gpt4o_cost_estimate = (words / 0.75) / 1000 * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]
...
def run_assistant(
    ...
    while True:
        ...
        stream = retry_with_backoff(_stream_call)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for item in results:
        row = (
            f"{short(item.get('prompt', ''))} | "
            f"{short(item.get('gpt4o_response', ''))} | "
            f"{short(item.get('mini_response', ''))} | "
            f"{item.get('gpt4o_latency', 0):.4f} | "
            f"{item.get('mini_latency', 0):.4f}"
        )
        lines.append(row)
